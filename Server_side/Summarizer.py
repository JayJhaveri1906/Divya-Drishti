...
class OurSummarizer:
    def __init__(self):
        ...
        self.summarizer = SumySummarizer(self.stemmer)
        self.summarizer.stop_words = get_stop_words(self.LANGUAGE)

    def genSummary(self, text):
        # parser = PlaintextParser.from_file("trump.txt", Tokenizer(LANGUAGE))
        # with open('trump.txt', 'r') as file:
        #     file_contents = file.read()
        #
        #     # print('Total words:   ', len(file_contents.split()))
        #     SENTENCES_COUNT = file_contents.count('.') // 2
        #     if SENTENCES_COUNT == 0:
        #         SENTENCES_COUNT = 1
        #     print(SENTENCES_COUNT)
        # print(parser.paragraphs)
        parser = PlaintextParser.from_string(text, Tokenizer(self.LANGUAGE))
        self.SENTENCES_COUNT = text.count('.') // 2
        if self.SENTENCES_COUNT == 0:
            self.SENTENCES_COUNT = 1
        print(self.SENTENCES_COUNT)

        finalStr = ""
        for sentence in self.summarizer(parser.document, self.SENTENCES_COUNT):
            # print(type(sentence))
            finalStr += str(sentence)
            finalStr += "\n"
        return finalStr
